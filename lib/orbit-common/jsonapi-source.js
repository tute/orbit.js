...
var JSONAPISource = Source.extend({
  ...
  _transform: function(operation) {
    ...
    // No need to update links to the API. Relations in the API are
    // guaranteed since object is created, and JSON API doesn't know how
    // to respond to these unnecessary calls.
    if (path.indexOf('__rel') >= 0) {
      console.log('!!!! [ORBIT] [JSON] Preventing update of relationship to the API', path);
      return;
    }
